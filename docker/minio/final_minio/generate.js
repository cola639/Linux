 ...
 */
const skipFolderArr = ["node_modules", ".git", "dist", "target"]; // customize
const skipFilesArr = [".DS_Store", "project-tree.md", "content.md"]; // customize
...
async function main() {
  // ✅ No need to input project path: always use current directory
  ...
  const outContent = path.join(root, "content.md");
...
async function isLikelyTextFile(filePath) {
  ...
  // Read first 2KB and check for NUL
  try {
